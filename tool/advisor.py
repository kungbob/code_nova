...
def message_lack_redundance (lack, redundance):
	...
	str = "There may be some error in your program. The reason may be the followings:\n" + "\n"

	if lack:
		str = str + "Lacking of: "
		for i in lack:
			str = str + conversion_list[i] + ", "
		str = str[:-2] + "\n"
		str = str + "You may try to add these element(s) to see whether the problem can be solved.\n\n"

	if redundance:
		str = str + "Redundance of: "
		for i in redundance:
			str = str + conversion_list[i] + ", "
		str = str[:-2] + "\n"
		str = str + "You may try to delete these element(s) to see whether the problem can be solved.\n\n"
...
def message_depth (flatten_version, flatten_problem):
	...
	if flatten_version['maxIfDepth'] != flatten_problem['maxIfDepth']:
		str = str + "Maximum depth of 'if' in your program is %d while the maximum depth of 'if' in the correct answer is %d. You may check the impletation of 'if' again.\n" % (flatten_version['maxIfDepth'], flatten_problem['maxIfDepth'])
		str = str + "\n"

	if flatten_version['maxLoopDepth'] != flatten_problem['maxLoopDepth']:
		str = str + "Maximum depth of loop in your program is %d while the maximum depth of loop in the correct answer is %d. You may check the impletation of loop again.\n" % (flatten_version['maxLoopDepth'], flatten_problem['maxLoopDepth'])
		str = str + "\n"

	if flatten_version['maxArrayDim'] != flatten_problem['maxArrayDim']:
		str = str + "Maximum dimension of arrray in your program is %d while the maximum dimension of array in the correct answer is %d. You may check the impletation of array again.\n" % (flatten_version['maxArrayDim'], flatten_problem['maxArrayDim'])
		str = str + "\n"
...
def advisor(version_tree, problem_tree, total_count):
	...
	#compare all elements in the compare_list
	...
	suggestions.append(message_lack_redundance(lack, redundance))
